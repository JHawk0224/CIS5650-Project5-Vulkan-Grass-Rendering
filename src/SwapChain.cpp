...
void SwapChain::Create(int w, int h) {
    ...
    if (queueFamilyIndices[QueueFlags::Graphics] != queueFamilyIndices[QueueFlags::Present]) {
        ...
        unsigned int indices[] = {static_cast<unsigned int>(queueFamilyIndices[QueueFlags::Graphics]),
...
uint32_t SwapChain::GetCount() const { return static_cast<uint32_t>(vkSwapChainImages.size()); }
...
bool SwapChain::Acquire() {
    ...
    VkResult result = vkAcquireNextImageKHR(device->GetVkDevice(), vkSwapChain, std::numeric_limits<uint64_t>::max(),
